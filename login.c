...
char *argv[] = { "sh", 0 };
int MAX_LEN = 200; // buffer length for username, password, and user id lengths
int NUM_OF_USERS = 4;
int STDIN  = 0;
int STDOUT = 1;
int STDERR = 2;
...
main(void)
{
   ...
   for (;;)
   {
	...
	// user is logged off
	// generates a new shell for the new user
	wait();
	printf(1, "Goodbye %s",username); // no newline needed b/c username contains one...
	loggedInUserId = -1;
	secondAndOnIteration = 0;
    }
