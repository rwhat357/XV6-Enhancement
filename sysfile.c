...
extern char* skipelem(char *path, char *name); //fs.c
...
int sys_fCopyFile(void)
{
  ...
  if(strncmp(pathsrc,pathdest,256) == 0){
    return -4;
  }

  begin_op();

  if ((sp = namei(pathsrc)) == 0)
  {
    end_op();
    return -1;
  }
  if ((dp = namei(pathdest)) == 0)
  {
    //doesn't exist
    if ((dp = create(pathdest, T_FILE, 0, 0)) == 0)
    {
      //couldn't be created
      end_op();
      return -2;
  ...
  cprintf("fCopyFile: done getting inodes\n");

  int n = 5;

  while(n > 0){

    cprintf("fCopyFile: offset:%d\n",datacounter);

    begin_op();
    ilock(sp);
      n = readi(sp, buffer, datacounter, sizeof(buffer));
    iunlock(sp);
    end_op();

    begin_op();
    ilock(dp);
      writei(dp, buffer, datacounter, n);
    iunlock(dp);
    end_op();
  ...
  cprintf("fCopyFile: done read/write loop\n");
...
int sys_fMoveFile(void)
{
  char *pathori;
  char *pathnew;
  char name[DIRSIZ];
  char destpath[255];
  struct inode *dp, *ip, *destip;
  struct dirent de;
  uint off;
  int result;

  if(argstr(0, &pathori) < 0 || argstr(1, &pathnew) < 0)
    return -10;

  if(namecmp(pathori, ".") == 0 || namecmp(pathori, "..") == 0)
    return -6;  //can't move "." or "..".

  if(strncmp(pathori,pathnew,256) == 0){
    return -7;  //source and destination same
  }

  begin_op();
  if((ip = namei(pathori)) == 0){
    end_op();
    return -1;	//original file not found
  }
  ...
  if(ip->type == T_DIR){
    iunlockput(ip);
    end_op();
    return -2;  //original file is a directory
  }
...
tryagain:
  ...
  if((dp = nameiparent(pathnew, name)) == 0){
    result = -8; //new path does not exist
    goto bad;
  }
  ilock(dp);

  // Check that name is not present.
  if((destip = dirlookup(dp, name, 0)) != 0){
    iunlockput(dp);
    ilock(destip);
    if(destip->type == T_DIR)
    {
      char *tmppath;
      iunlockput(destip);
      memset(destpath,0,255);
      strncpy(destpath,pathori,strlen(pathori));
      tmppath = (char*)&destpath;
      while ((tmppath = skipelem(tmppath,name)) != 0)
        {};
      memset(destpath,0,255);
      strncpy(destpath,pathnew,strlen(pathnew));
      destpath[strlen(pathnew)] = '/';
      strncpy(destpath+strlen(destpath),name,strlen(name));
      pathnew = (char*)&destpath;
      goto tryagain;
      ...
      if(destpath[0] == 0){
        ...
        result = -9; //file in adjusted path already exists
      }
      goto bad;
    }
  }
  ...
  if(dp->dev != ip->dev || dirlink(dp, name, ip->inum) < 0){
    iunlockput(dp);
    result = -4; //creating a new link failed
    goto bad;
  }
  iunlockput(dp);

  if((dp = nameiparent(pathori, name)) == 0){
    result = -5; //original file doesn't have a parent inode
    goto bad;
  }
  ilock(dp);
